 ...
 * </p>
 * <p>
 * Description: Panel used to contains input controls. It can set input control
 * values and retrieve from them the values to set the model (the value object).
 * It can also remotely read data and set the v.o. (and the input controls).
 * </p>
 * <p>
 ...
 * @version 1.0
 */
public class Form extends JPanel implements DataController,
		ValueChangeListener, GenericButtonController, ActionListener {
	...
	/**
	 * linked panels, related to this (optional); they will receive pull/store
	 * events fired to this
	 */
	...
	/**
	 * flag used to define if this Form panel currently has focus (used to
	 * listen for acceleration keys events to dispatch to toolbar buttons of the
	 * form)
	 */
	...
	/**
	 * LinkButton objects, that inherit abilitation state of the other form
	 * buttons
	 */
	private ArrayList linkButtons = new ArrayList();

	/**
	 * collection of input controls attribute names to disable when the
	 * specified attribute will be setted to the specified value; pairs of type
	 * (attribute name, List of AttributeCouple objects)
	 */
	private Hashtable inputControlsToDisable = new Hashtable();

	/**
	 * collection of GenericButton objects to disable when the specified
	 * attribute will be setted to the specified value; pairs of type
	 * (GenericButton object, List of GenericButtonController objects)
	 */
	private Hashtable buttonsToDisable = new Hashtable();

	/**
	 * collection of input controls linked to this form; couples of type:
	 * attribute name, List of InputControl objects
	 */
	...
	/**
	 * navigation bar linked to the current Form (optional); if this property is
	 * defined then Form data loading is automatically performed when using
	 * navigator bar; also grid selection row is updated
	 */
	private NavigatorBar navBar = null;

	/**
	 * attributes that must be defined both on grid v.o. and on the Form v.o.
	 * used to select on grid the (first) row that matches pk values
	 */
	private HashSet pkAttributes = null;

	/**
	 * flag used to enable Form data reloading when clicking with the left mouse
	 * button onto the grid
	 */
	private boolean reloadModelWhenClickingWithMouse;

	/**
	 * flag used to enable Form data reloading when pressing up/down keys onto
	 * the grid
	 */
	private boolean reloadModelWhenPressingKey;

	/**
	 * flag used to define if an inner v.o. must be automatically instantiated
	 * when a setter method is invoked; default value: <code>true</code>
	 */
	private boolean createInnerVO = true;

	/**
	 * collection of GenericButtons binded to this Form, i.e. whose text is
	 * setted with the attribute value; pairs of type (attribute name, List of
	 * GenericButtons objects)
	 */
	private Hashtable bindedGenericButtons = new Hashtable();

	/**
	 * collection of LinkButtons binded to this Form, i.e. whose
	 * text/tooltip/uri is setted with the attribute value; pairs of type
	 * (attribute name, List of GenericButtons objects)
	 */
	private Hashtable bindedLinkButtons = new Hashtable();

	/** key listener used to listen for shortcut events */
	private FormShortcutsListener shortcutsListener = new FormShortcutsListener(
			this);
	...
	/**
	 * current enabled button state, before GenericButton.setEnabled method
	 * calling
	 */
	...
	/**
	 * collection of attributes to not manage because they are related to lazy
	 * initialized inner objects
	 */
	...
	/**
	 * @return collection of attributes to not manage because they are related
	 *         to lazy initialized inner objects
	 */
	...
	/**
	 * @return define if this Form panel currently has focus (used to listen for
	 *         acceleration keys events to dispatch to toolbar buttons of the
	 *         form)
	 */
	 ...
	 * 
	 * @param button
	 *            generic button that fires this event
	 * @param currentValue
	...
	public final void setFocusOnForm() {
		...
		try {
			...
			KeyListener[] ll = ApplicationEventQueue.getInstance()
					.getKeyListeners();
			for (int i = 0; i < ll.length; i++)
				if (ll[i] instanceof FormShortcutsListener)
					((FormShortcutsListener) ll[i]).getForm()
							.dropFocusFromForm();
		} catch (Exception ex1) {
		}
		...
		if (ClientSettings.SHOW_FOCUS_BORDER_ON_FORM)
			// set "focus" border...
			setBorder(BorderFactory.createCompoundBorder(getNotFocusedBorder(),
					BorderFactory.createLineBorder(
							ClientSettings.FORM_FOCUS_BORDER, 1)));
	...
	public final void dropFocusFromForm() {
		...
		ApplicationEventQueue.getInstance()
				.removeKeyListener(shortcutsListener);
			...
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					if (navBar != null) {
						...
						// used to correctly set the row in grid related to the
						// current v.o. in the Form...
						navBar.addBeforeActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent e) {
								int rowInGrid = getRowIndexInGrid();
								if (rowInGrid != -1)
									Form.this.grid.setRowSelectionInterval(
											rowInGrid, rowInGrid);
							}
						});
					}
	...
	/**
	 * @param comp
	 *            form which may be contained into a JFrame
	 * @return JFrame object which contains the graphic component; null if comp
	 *         is not inside a JFrame
	 */
	...
	/**
	 * @param comp
	 *            form which may be contained into a JInternalFrame
	 * @return JInternalFrame object which contains the graphic component; null
	 *         if comp is not inside a JInternalFrame
	 */
	...
	public final void setEditButton(EditButton editButton) {
		...
		if (editButton != null) {
			...
			editButton.setToolTipText(ClientSettings.getInstance()
					.getResources().getResource("Edit record (CTRL+E)"));
	...
	public final void setInsertButton(InsertButton insertButton) {
		...
		if (insertButton != null) {
			...
			insertButton.setToolTipText(ClientSettings.getInstance()
					.getResources().getResource("New record (CTRL+I)"));
	...
	public final void setCopyButton(CopyButton copyButton) {
		...
		if (copyButton != null) {
			...
			copyButton.setToolTipText(ClientSettings.getInstance()
					.getResources().getResource("Copy record (CTRL+C)"));
	...
	/**
	 * Set refresh/cancel button linked to this form. It allows to cancel
	 * insert/edit operation and reloads the previous data in the form and set
	 * it in READONLY mode.
	 * 
	 * @param ReloadButton
	 *            refresh/cancel button
	...
	public final void setReloadButton(ReloadButton reloadButton) {
		...
		if (reloadButton != null) {
			...
			reloadButton.setToolTipText(ClientSettings
					.getInstance()
					.getResources()
					.getResource(
	...
	public final void setDeleteButton(DeleteButton deleteButton) {
		...
		if (deleteButton != null) {
			...
			deleteButton.setToolTipText(ClientSettings.getInstance()
					.getResources().getResource("Delete record (CTRL+D)"));
	...
	/**
	 * Set save button linked to this form. It allows to create an insert/edit
	 * sql instruction
	 * 
	 * @param saveButton
	 *            save button
	...
	public final void setSaveButton(SaveButton saveButton) {
		...
		if (saveButton != null) {
			...
			saveButton.setToolTipText(ClientSettings.getInstance()
					.getResources().getResource("Save record (CTRL+S)"));
			bindedButtons.add(saveButton);
		}
	}

	/**
	 * Method called from refresh/cancel button: it reload the v.o. (in another
	 * thread).
	 */
	private void reloadData() {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				// it will be used another thread to do not block the rest of
				// the application (the main graphical thread)...
				if (reloadButton != null)
				...
				boolean[] oldGenericButtonValues = new boolean[genericButtons
						.size()];
				for (int i = 0; i < genericButtons.size(); i++) {
					oldGenericButtonValues[i] = ((GenericButton) genericButtons
							.get(i)).isEnabled();
					((GenericButton) genericButtons.get(i)).setEnabled(false);
				}
				// boolean[] oldLinkButtonValues = new
				// boolean[linkButtons.size()];
				for (int i = 0; i < linkButtons.size(); i++) {
					// oldLinkButtonValues[i] =
					// ((LinkButton)linkButtons.get(i)).getOldValue();
				...
				if (errorOnLoad) {
					...
					// // set toolbar buttons state in the state previous to the
					// insert/edit.
					// if (editButton!=null)
					...
					for (int i = 0; i < genericButtons.size(); i++)
						((GenericButton) genericButtons.get(i))
								.setEnabled(oldGenericButtonValues[i]);
					...
					for (int i = 0; i < linkButtons.size(); i++)
						...
						((LinkButton) linkButtons.get(i))
								.setEnabled(((LinkButton) linkButtons.get(i))
										.getOldValue());
	...
	private boolean loadData() {
		try {
			...
			try {
				ClientUtils.getParentWindow(this).setCursor(
			...
			if (!answer.isError()) {
				model.setValueObject((ValueObject) ((VOResponse) answer)
						.getVo());
				previousVO = getVOModel().getValueObject();
			} else
				OptionPane.showMessageDialog(this, ClientSettings.getInstance()
						.getResources()
						.getResource("Error while loading data:")
						+ "\n"
						+ ClientSettings.getInstance().getResources()
								.getResource(answer.getErrorMessage()),
						ClientSettings.getInstance().getResources()
								.getResource("Error on Loading"),
	...
	private void maybeCreateVOModel() {
		try {
			...
			Logger.error(this.getClass().getName(), "maybeCreateVOModel",
					"Error on setting Form data model:\n" + ex.toString(), null);
		}
	}
	 ...
	 * 
	 * @param mode
	 *            form mode; possible values: READONLY, INSERT, EDIT
	 */
	public final void setMode(int mode) {
		if (Beans.isDesignTime())
			return;
		if (mode != Consts.READONLY && mode != Consts.EDIT
				&& mode != Consts.INSERT)
		...
		if (mode == Consts.INSERT) {
			try {
				...
				ValueObject vo = (ValueObject) model.getValueObjectType()
						.newInstance();
				...
				if (ClientSettings.PRESET_LAST_VALUE_IN_COMBO_CONTROL) {
					// evaluate each combo control and preset last value if the
							...
							for (int i = 0; i < list.size(); i++) {
								...
								if (comp instanceof ComboBoxControl
									...
									value = model.getValue(
											...
											model.getOldValueObject());
									model.setValue(comp.getAttributeName(),
											value);
								}
							}
						}
			...
			try {
				previousVO = (ValueObject) ((ValueObject) model
						.getValueObject()).clone();
			} catch (CloneNotSupportedException ex1) {
				Logger.error(this.getClass().getName(), "setMode",
						"Error while duplicating the value object", ex1);
			}
	...
	/**
	 * @return <code>true</code> if there exists at least one input control that
	 ...
	 * 
	 * @param c
	 *            components whose edit state must be set
	 * @param enabled
	 *            edit state
	 */
	private void setEnabled(Component[] c, boolean enabled) {
		...
		for (int i = 0; i < c.length; i++) {
			if (c[i] instanceof JScrollPane) {
				setEnabled(((JScrollPane) c[i]).getViewport().getComponents(),
						enabled);
				continue;
			}
			...
			if (!(c[i] instanceof JLabel || c[i] instanceof JScrollPane)) {
				if (c[i] instanceof InputControl && enabled) {
					if (((InputControl) c[i]).isEnabledOnInsert()
							&& getMode() == Consts.INSERT) {
						canEdit = isInputControlEnabled(enabled,
								(InputControl) c[i]);
						...
						if (canEdit
								...
								&& ((ComboBoxControl) c[i]).getComboBox()
										.getSelectedIndex() == -1
								&& ((ComboBoxControl) c[i]).getComboBox()
										.getItemCount() > 0)
							((ComboBoxControl) c[i]).getComboBox()
									.setSelectedIndex(0);
					} else if (((InputControl) c[i]).isEnabledOnEdit()
							&& getMode() == Consts.EDIT)
						c[i].setEnabled(isInputControlEnabled(enabled,
								(InputControl) c[i]));
					else if (enabled
							&& (getMode() == Consts.INSERT || getMode() == Consts.EDIT))
	...
	/**
	 * Method called by setEnabled method to check if input control must be
	 * disabled, independently from enabledOnInsert/Edit settings.
	 * 
	 * @return <code>true</code> if input control does not match any input
	 *         controls disabilitation policies, <code>false</code> if there
	 *         exist a policy that matches
	 */
	private boolean isInputControlEnabled(boolean enabled, InputControl control) {
		...
		maybeCreateVOModel();
		ArrayList list = (ArrayList) inputControlsToDisable.get(control
				.getAttributeName());
		...
		if (list != null)
			for (int i = 0; i < list.size(); i++) {
				...
				if (stateValue != null && ac.getAttributeValue() != null
						&& stateValue.equals(ac.getAttributeValue())
	...
	/**
	 * Define input controls attribute names that will be disabled
	 * (independently from enabledOnInsert/Edit settings) when the specified
	 * attribute will be set to the specified value.
	 * 
	 * @param attributeNames
	 *            collections of attribute names related to input controls to
	 *            disable
	 * @param stateAttributeName
	 *            attribute name in the form value object to test
	 * @param state
	 *            attribute value in the form value object to test: if the value
	 *            is this one, then input controls will be disabled (in
	 *            INSERT/EDIT modes)
	 */
	public final void addInputControlAttributesNotEditableOnState(
			HashSet attributeNames, String stateAttributeName, Object state) {
	...
	/**
	 * Define GenericButton objects linked to this that will be disabled
	 * (independently from the form mode) when the specified attribute will be
	 * set to the specified value.
	 * 
	 * @param buttons
	 *            collections GenericButton objects linked to this that have to
	 *            be disabled
	 * @param buttonController
	 *            interface that defines button disabilitation, according to
	 *            some custom policy
	 */
	public final void addButtonsNotEnabled(HashSet buttons,
	...
	/**
	 * Method called by GenericButton.setEnabled method to check if the button
	 * must be disabled.
	 * 
	 * @param button
	 *            button whose abilitation must be checked
	 * @return <code>true</code> if no policy is defined in the form for the
	 *         specified button, <code>false</code> if there exists a
	 *         disabilitation policy for the specified button (through
	 *         addButtonsNotEnabledOnState form method)
	 */
	...
	public final void reload() {
		...
		if (getMode() != Consts.READONLY) {
			...
			if (isChanged())
				// data in changed: show message dialog to confirm the
				// refresh/cancel operation...
				canReload = OptionPane
						.showConfirmDialog(
								this,
								ClientSettings
										.getInstance()
								...
								ClientSettings.getInstance().getResources()
										.getResource("Attention"),
								JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION;
			if (canReload) {
				if (!ClientSettings.RELOAD_LAST_VO_ON_FORM
						&& getMode() == Consts.INSERT)
	...
	/**
	 * This method is called by reload method to reload data. Developer can
	 * explicity call this method to force data model reloading.
	 */
	public final void executeReload() {
		try {
			// check if there is a linked grid: in this case update selected row
			// in grid BEFORE reloading form!
	...
	public final void insert() {
		if (getMode() == Consts.READONLY) {
			...
			Logger.error(this.getClass().getName(), "insert",
					"The form is not in READONLY mode: operation not allowed.",
					null);
	}
	...
	public final void copy() {
		...
		if (getMode() == Consts.READONLY) {
			try {
				...
				ValueObject vo = (ValueObject) ((ValueObject) model
						.getValueObject()).clone();
				...
				// fill in the new v.o. with the duplicable attributes of the
				// cloned v.o...
				...
				Logger.error(this.getClass().getName(), "copy",
						"Error while duplicating the Form content", ex);
			}
		} else
			Logger.error(this.getClass().getName(), "copy",
					"The form is not in READONLY mode: operation not allowed.",
	 ...
	 */
	private void copyInputControlValues(ValueObject vo, Form form,
			Component[] c, boolean evalLinkedForm) throws Throwable {
		maybeCreateVOModel();
		...
		for (int i = 0; i < c.length; i++)
			if (c[i] instanceof InputControl) {
					...
					try {
						attributeValue = vo
								...
								.getMethod(
										...
										new Class[0]).invoke(vo, new Object[0]);
									...
									.getMethod(
											"is"
													...
													+ attributeName
															.substring(1),
											new Class[0])
									.invoke(vo, new Object[0]);
						} catch (NoSuchMethodException exx) {
							attributeValue = getVOModel().getValue(
									attributeName, vo);
						}
					}
				...
				copyInputControlValues(vo, form,
						((Container) c[i]).getComponents(), false);

		Container container = null;
		if (evalLinkedForm)
			for (int i = 0; i < linkedPanels.size(); i++) {
				container = (Container) linkedPanels.get(i);
				copyInputControlValues(vo, form, container.getComponents(),
						false);
			}
	}
	...
	public final void edit() {
		if (getMode() == Consts.READONLY) {
			...
			Logger.error(this.getClass().getName(), "edit",
					"The form is not in READONLY mode: operation not allowed.",
					null);
	}
	...
	public final void delete() {
		if (getMode() == Consts.READONLY) {
			...
			if (OptionPane.showConfirmDialog(
					this,
					ClientSettings.getInstance().getResources()
							.getResource("Confirm deliting data?"),
					ClientSettings.getInstance().getResources()
							.getResource("Attention"),
					JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
				try {
					maybeCreateVOModel();
					Response response = this.formController
							.deleteRecord((ValueObject) model.getValueObject());
					if (!response.isError()) {
						...
						// if a grid has been linked to this Form then update
						// automatically its content:
						// the removed record will be dropped from the grid...
						if (grid != null && pkAttributes != null) {
							if (gridRowIndex != -1)
								grid.getVOListTableModel().removeObjectAt(
										gridRowIndex);
						}
			...
			Logger.error(this.getClass().getName(), "delete",
					"The form is not in READONLY mode: operation not allowed.",
					null);
	}

	/**
	 * @return list of input controls not in valid state, i.e. the collection of
	 *         input controls that are mandatory and have a null value
	 */
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if (comp instanceof SpinnerNumberControl
						...
						if (comp instanceof CodLookupControl) {
							if (((CodLookupControl) comp).getCodBox()
									.hasFocus())
								((CodLookupControl) comp).getCodBox()
										.forceValidate();
							if (((CodLookupControl) comp).getLookupController() != null
									&& !((CodLookupControl) comp)
											.getLookupController()
											.isCodeValid())
								inputControlsNotValid.add(comp);
						}
	...
	/**
	 * @return list of labels of input controls not in valid state, i.e. the
	 *         collection of input controls that are mandatory and have a null
	 *         value
	 */
	...
	/**
	 * If all input controls linked to the form are in a valid state, then push
	 * each value to its associated data model.
	 * 
	 * @return boolean indicating whether or not the input control values were
	 *         successfully pushed to the data model
	 */
		...
		try {
			if (set.size() > 0) {
				...
				OptionPane.showMessageDialog(this,
						ClientSettings.getInstance().getResources()
								.getResource("Error while validating data:")
								+ "\n" + list,
						ClientSettings.getInstance().getResources()
								.getResource("Validation Error"),
						JOptionPane.WARNING_MESSAGE);
				return false;
			}
				...
				if (list != null)
					for (int i = 0; i < list.size(); i++) {
						...
						if (comp instanceof ProgressBarControl)
							...
							if (((BaseInputControl) comp).getBindingComponent()
									.hasFocus()) {
								((BaseInputControl) comp).getBindingComponent()
										.transferFocus();
								try {
			...
			if (set.size() > 0)
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						...
						findInputControlsAndButtons(Form.this.getComponents(),
								true, inputControls, buttons);
						for (int i = 0; i < inputControls.size(); i++)
							if (set.contains(inputControls.get(i))) {
								if (inputControls.get(i) instanceof BaseInputControl) {
									if (!((BaseInputControl) inputControls
											.get(i)).getBindingComponent()
											.hasFocus())
										((BaseInputControl) inputControls
												.get(i)).getBindingComponent()
												.requestFocus();
								} else {
									if (!((Component) inputControls.get(i))
											.hasFocus())
										((Component) inputControls.get(i))
												.requestFocus();
								}
								break;
							}
	...
	/**
	 * Set the content for each linked input control, based on value object
	 * content.
	 * 
	 * @return boolean indicating whether or not the input controls are all
	 *         correctly setted
	 */
	...
	/**
	 * Set the content for the specified linked input control, based on value
	 * object content.
	 * 
	 * @return boolean indicating whether or not the input control has been
	 *         correctly setted, according to the specified attribute name
	 */
	public final boolean pull(String attributeName) {
		...
		if (clazz != null && ValueObject.class.isAssignableFrom(clazz)) {
			// if the value just changed is an inner value object, then all its
			// attributes are managed as changed:
			// check if there are input controls binded to these changed
			// attributes and for each one "pull" it...
			Enumeration en = bindings.keys();
			while (en.hasMoreElements()) {
				attrName = en.nextElement().toString();
		...
		if (list != null) {
			...
			for (int i = 0; i < list.size(); i++) {
				btn = (LinkButton) list.get(i);
				try {
					if (btn.getLabelAttributeName() != null
							&& !btn.getLabelAttributeName().equals("")) {
						if (model.getValue(btn.getLabelAttributeName()) != null)
							btn.setLabel(model.getValue(
									btn.getLabelAttributeName()).toString());
						else
							btn.setLabel("");
					}
					if (btn.getTooltipAttributeName() != null
							&& !btn.getTooltipAttributeName().equals("")) {
						if (model.getValue(btn.getTooltipAttributeName()) != null)
							btn.setToolTipText(model.getValue(
									btn.getTooltipAttributeName()).toString());
						else
							btn.setToolTipText("");
					}
					if (btn.getUriAttributeName() != null
							&& !btn.getUriAttributeName().equals("")) {
						if (model.getValue(btn.getUriAttributeName()) != null)
							btn.setUri(model
									.getValue(btn.getUriAttributeName())
									.toString());
						else
							btn.setUri("");
					}
	...
	/**
	 * Method called by save button. If an error occours while saving data, an
	 * error message is showed.
	 * 
	 * @return <code>true</code> if data saving is successfully completed,
	 *         <code>false</code> if an error occours
	 */
	public final boolean save() {
		...
		if (getMode() != Consts.READONLY) {
			...
			try {
				...
				if (getMode() == Consts.INSERT) {
					if (!formController.beforeSaveDataInInsert(this))
						return false;
					response = formController.insertRecord((ValueObject) model
							.getValueObject());
				} else if (getMode() == Consts.EDIT) {
					if (!formController.beforeSaveDataInEdit(this))
						return false;
					response = formController.updateRecord(previousVO,
							(ValueObject) model.getValueObject());
				}
				if (!response.isError()) {
					model.setValueObject((ValueObject) ((VOResponse) response)
							.getVo());
					if (getMode() == Consts.INSERT)
						previousVO = getVOModel().getValueObject();

					// reset input controls value according to value object
					// content (which could be filled on the server side)
					pull();
					...
					try {
						...
						// in case in afterInsert/Edit the form is closed,
						// bindings variable will be null!
						return false;
					}
					return true;
				} else {
					 ...
					 * "Error while saving: incorrect data.")+"\n"+
					 * ClientSettings
					 * .getInstance().getResources().getResource(response
					 * .getErrorMessage()),
					 * ClientSettings.getInstance().getResources
					 * ().getResource("Saving Error"),
					 * JOptionPane.WARNING_MESSAGE );
					...
					ErrorInfo info = new ErrorInfo(
							...
							(resp.getException() == null) ? ClientSettings.getInstance().getResources().getResource(resp.getErrorMessage()) : resp.getException().getMessage(),
							null,
							null,
							resp.getException(),
							(resp.getLevel() == ErrorResponse.WARNING) ? ErrorLevel.WARNING : ErrorLevel.SEVERE,
							null);
			...
			Logger.error(this.getClass().getName(), "save",
					"The form is in READONLY mode: operation not allowed.",
					null);
			return false;
		}
	}
	...
	/**
	 * If a grid has been linked to this Form then update automatically its
	 * content: the new/updated row will be reloaded and set into the grid...
	 */
		...
		try {
			if (grid != null && pkAttributes != null) {
				...
				while (it.hasNext()) {
					...
					filter[0] = new FilterWhereClause(pkAttrName, Consts.EQ,
							model.getValue(pkAttrName));
					filters.put(pkAttrName, filter);
				}
				Response res = grid.getGridDataLocator().loadData(
						GridParams.NEXT_BLOCK_ACTION, 0, filters,
				...
				if (res.isError())
					Logger.error(
							...
							"save",
							"Error while loading new row for grid:\n"
					...
					if (rows.size() == 1) {
						...
						if (gridRowIndex == -1) {
							grid.getVOListTableModel().addObject(
									(ValueObject) rows.get(0));
							grid.repaint();
							SwingUtilities.invokeLater(new Runnable() {
								public void run() {
									grid.setRowSelectionInterval(grid
											.getVOListTableModel()
											.getRowCount() - 1, grid
											.getVOListTableModel()
											.getRowCount() - 1);
							...
							grid.getVOListTableModel().updateObjectAt(
									(ValueObject) rows.get(0), gridRowIndex);
							grid.repaint();
							SwingUtilities.invokeLater(new Runnable() {
								public void run() {
									grid.setRowSelectionInterval(gridRowIndex,
											gridRowIndex);
								}
							});
						}
					...
					// if this Form is a nested component of a grid, then
					// refresh grid row...
	...
	/**
	 * @return parent Grid that has this as nested grid; null if this is a main
	 *         grid, not a nested grid
	 */
	...
	public final String getVOClassName() {
		try {
			if (Beans.isDesignTime())
				AttributeNameEditor
						.setDesignVOClass(Class.forName(voClassName));
		} catch (Throwable ex) {
		}
	...
	public final void setVOClassName(String voClassName) {
		...
		// -MC 18/09/2008: moved to addNotify method, in order to store
		// "createInnerVo" property to pass to new VOModel(...)
		// try {
		// this.model = new
		// VOModel(Class.forName(voClassName),createInnerVO,this);
		// }
		// catch (Throwable ex) {
		// Logger.error(this.getClass().getName(), "setVOClassName",
		// "Error on setting Form data model:\n"+ex.toString(),null);
		// }
	}
	...
	// }
	// catch (Exception ex) {
	// Logger.error(this.getClass().getName(), "setVOModel",
	// "Error on setting Form data model:\n"+ex.toString(),null);
	// }
	// }
	...
	// * only input controls of type InputControl that have an attribute name
	// defined are linked.
	// * @param form form to use to link the input controls
	// * @param c components added to the container
	// */
	// private void linkInputControlsAndButtons(Component[] c,boolean
	// evalLinkedForm) {
	// maybeCreateVOModel();
	// for(int i=0;i<c.length;i++)
	// if (c[i] instanceof InputControl)
	...
	// // input control not linkable: it has not defined the attribute name
	// property...
	// continue;
	// try {
	// // bind input control...
	...
	// }
	// catch (Exception ex) {
	// Logger.error(this.getClass().getName(), "linkInputControls",
	// "Error while linking the input control having attribute name '"+((InputControl)c[i]).getAttributeName()+"'",ex);
	// }
	// }
	// catch (Exception ex) {
	// ex.printStackTrace();
	...
	// ArrayList list =
	// (ArrayList)bindedGenericButtons.get(((GenericButton)c[i]).getAttributeName());
	// if (list==null)
	// list = new ArrayList();
	// list.add(c[i]);
	...
	// ((LinkButton)c[i]).getLabelAttributeName()!=null &&
	// !((LinkButton)c[i]).getLabelAttributeName().equals("") ||
	// ((LinkButton)c[i]).getTooltipAttributeName()!=null &&
	// !((LinkButton)c[i]).getTooltipAttributeName().equals("") ||
	// ((LinkButton)c[i]).getUriAttributeName()!=null &&
	// !((LinkButton)c[i]).getUriAttributeName().equals("")
	// )) {
	// final String labelAttributeName =
	// ((LinkButton)c[i]).getLabelAttributeName();
	// final String tooltipAttributeName =
	// ((LinkButton)c[i]).getTooltipAttributeName();
	...
	/**
	 * Bind input controls added to the specified container: only input controls
	 * of type InputControl that have an attribute name defined are linked.
	 * 
	 * @param form
	 *            form to use to link the input controls
	 * @param c
	 *            components added to the container
	 */
	private void linkInputControlsAndButtons(Component[] c,
			boolean evalLinkedForm) {
		maybeCreateVOModel();
		ArrayList inputControls = new ArrayList();
		ArrayList buttons = new ArrayList();
		...
		for (int k = 0; k < inputControls.size(); k++) {
			try {
				...
				Logger.error(
						this.getClass().getName(),
						"linkInputControls",
						"Error while linking the input control having attribute name '"
								+ ((InputControl) inputControls.get(k))
										.getAttributeName() + "'", ex);
			}
		}
		...
		for (int k = 0; k < buttons.size(); k++) {
			if (buttons.get(k) instanceof GenericButton) {
				ArrayList list = (ArrayList) bindedGenericButtons
						.get(((GenericButton) buttons.get(k))
								.getAttributeName());
				if (list == null)
					list = new ArrayList();
				...
				bindedGenericButtons.put(
						...
						list);
				final ArrayList auxList = list;
				final String attributeName = ((GenericButton) buttons.get(k))
						.getAttributeName();
				model.addValueChangeListener(new ValueChangeListener() {
					...
					public void valueChanged(ValueChangeEvent e) {
						if (e.getAttributeName().equals(attributeName)) {
							for (int j = 0; j < auxList.size(); j++)
								if (e.getNewValue() != null)
									((GenericButton) auxList.get(j)).setText(e
											.getNewValue().toString());
								else
									((GenericButton) auxList.get(j))
											.setText("");
						}
					}
				...
				final String labelAttributeName = ((LinkButton) buttons.get(k))
						.getLabelAttributeName();
				final String tooltipAttributeName = ((LinkButton) buttons
						.get(k)).getTooltipAttributeName();
				final String uriAttributeName = ((LinkButton) buttons.get(k))
						.getUriAttributeName();
				...
				if (list == null)
					list = new ArrayList();
				list.add(buttons.get(k));
				bindedLinkButtons.put(attrName, list);
				final ArrayList auxList = list;
				model.addValueChangeListener(new ValueChangeListener() {
					...
					public void valueChanged(ValueChangeEvent e) {
						if (e.getAttributeName().equals(labelAttributeName)) {
							for (int j = 0; j < auxList.size(); j++)
								if (e.getNewValue() != null)
									((LinkButton) auxList.get(j)).setLabel(e
											.getNewValue().toString());
								else
									((LinkButton) auxList.get(j)).setLabel("");
						} else if (e.getAttributeName().equals(
								tooltipAttributeName)) {
							for (int j = 0; j < auxList.size(); j++)
								if (e.getNewValue() != null)
									((LinkButton) auxList.get(j))
											.setToolTipText(e.getNewValue()
													.toString());
								else
									((LinkButton) auxList.get(j))
											.setToolTipText("");
						} else if (e.getAttributeName()
								.equals(uriAttributeName)) {
							for (int j = 0; j < auxList.size(); j++)
								if (e.getNewValue() != null)
									((LinkButton) auxList.get(j)).setUri(e
											.getNewValue().toString());
								else
									((LinkButton) auxList.get(j)).setUri("");
						}
	...
	/**
	 * Find input controls and buttons included in the specified container: only
	 * input controls of type InputControl that have an attribute name defined
	 * are linked.
	 * 
	 * @param c
	 *            components added to the container
	 * @param evalLinkedForm
	 *            analyze also linked Forms
	 */
	private void findInputControlsAndButtons(Component[] c,
			boolean evalLinkedForm, ArrayList inputControls, ArrayList buttons) {
		for (int i = 0; i < c.length; i++)
			if (c[i] instanceof InputControl)
				try {
					if (((InputControl) c[i]).getAttributeName() == null)
						// input control not linkable: it has not defined the
				...
				buttons.add(c[i]);
					...
					&& (((LinkButton) c[i]).getLabelAttributeName() != null
							...
							|| ((LinkButton) c[i]).getTooltipAttributeName() != null
							&& !((LinkButton) c[i]).getTooltipAttributeName()
									.equals("") || ((LinkButton) c[i])
							.getUriAttributeName() != null
							&& !((LinkButton) c[i]).getUriAttributeName()
									.equals(""))) {
				buttons.add(c[i]);
			} else if (c[i] instanceof Container)
				findInputControlsAndButtons(((Container) c[i]).getComponents(),
						false, inputControls, buttons);

		if (evalLinkedForm) {
			Container container = null;
			for (int i = 0; i < linkedPanels.size(); i++) {
				container = (Container) linkedPanels.get(i);
				findInputControlsAndButtons(container.getComponents(), false,
						inputControls, buttons);
			}
		}
	}
				...
				for (int i = 0; i < list.size(); i++) {
					try {
						...
						Logger.error(
								this.getClass().getName(),
								"removeLinkedPanel",
								"Error while unbinding the input control having attribute name '"
										+ ((InputControl) list.get(i))
												.getAttributeName() + "'", ex);
					}
				}
		}
	...
	/**
	 * Add a linked panel, that will receive the same events of this form (in
	 * pull/store operations).
	 * 
	 * @param c
	 *            container to link
	 */
	 ...
	 * @param c
	 *            panel to check
	 * @return <code>true</code> if "panel" is a linked panel,
	...
	/**
	 * Clean up fields content. Note: this method can be invoked only in
	 * INSERT/EDIT modes.
	 */
	...
	/**
	 * Clean up fields content, for each field or editable only fields. Note:
	 * this method can be invoked only in INSERT/EDIT modes.
	 * 
	 * @param cleanUpAlsoNotEditableFields
	 *            define if all fields must be clean up
	 */
	public final void cleanUp(boolean cleanUpAlsoNotEditableFields) {
		if (getMode() != Consts.INSERT && getMode() != Consts.EDIT) {
			Logger.error(this.getClass().getName(), "cleanUp",
					"You are not allowed to clean up fields in READONLY mode.",
					null);
		} else {
			Enumeration en = bindings.keys();
			String attrName = null;
					...
					for (int i = 0; i < list.size(); i++) {
						...
						try {
							if (cleanUpAlsoNotEditableFields
									|| getMode() == Consts.INSERT
									&& comp.isEnabledOnInsert()
									|| getMode() == Consts.EDIT
									&& comp.isEnabledOnEdit())
								comp.setValue(null);
						} catch (Exception ex) {
							Logger.error(this.getClass().getName(), "cleanUp",
									"Error on setting value to the input control having the attribute name '"
											+ comp.getAttributeName() + "'", ex);
						}
					}
				}
			}
	...
	public final void bind(InputControl comp) throws Exception {
		if (comp.getAttributeName() == null)
			throw new Exception(
					"No attribute name defined for the specified input control");
	...
	/**
	 * Remove an existing binding between the specified input control and this
	 * data model
	 * 
	 * @param comp
	 ...
	 * 
	 * @param e
	 *            ValueChangeEvent describing the event
	 */
	 ...
	 * 
	 * @param functionId
	 *            identifier associated to the container
	 */
	...
	/**
	 * Add a generic button, that inherits abilitation state of the other form
	 * buttons.
	 * 
	 * @param b
	 *            generic button, that inherits abilitation state of the other
	 *            form buttons
	 */
	...
	/**
	 * Remove a generic button, that inherits abilitation state of the other
	 * form buttons.
	 * 
	 * @param b
	 *            generic button, that inherits abilitation state of the other
	...
	/**
	 * Add a link button, that inherits abilitation state of the other form
	 * buttons.
	 * 
	 * @param b
	 *            link button, that inherits abilitation state of the other form
	 *            buttons
	 */
	...
	/**
	 * Remove a link button, that inherits abilitation state of the other form
	 * buttons.
	 * 
	 * @param b
	 *            link button, that inherits abilitation state of the other form
	...
	/**
	 * Link the specified grid control to the current Form, so that: - row
	 * selection event (fired by grid navigator bar) will force the Form data
	 * loading (ONLY IF loadModelWhenSelectingOnGrid is set to <code>true</code>
	 ...
	 * delete existing data on the Form will refresh grid content by removing
	 * the related row To correctly identify the row on grid to change a
	 * matching beetween attributes (specified in "pkAttributes" argument) of
	 * grid v.o. and Form v.o. is performed: it will be identified the first row
	 * on grid that matches this filter criteria, based on attribute values.
	 * 
	 * NOTE: "reloadModelWhenSelectingOnGrid" is set to <code>true</code> then
	 * Form data reloading is performed also when clicking with the left mouse
	 * button onto the grid and when pressing up/down keys onto the grid.
	 * 
	 * @param grid
	 *            grid control linked to the current Form, to update grid
	 *            content
	 * @param pkAttributes
	 *            attributes that must be defined both on grid v.o. and on the
	 *            Form v.o. used to select on grid the (first) row that matches
	 *            pk values; if the grid v.o. is the same class or a super-class
	 *            of Form v.o. then this HashSet could
	 * @param reloadModelWhenSelectingOnGrid
	 *            <code>true</code> to force data Form reloading when changing
	 *            row selection on grid by grid navigator bar
	 */
	public final void linkGrid(GridControl grid, HashSet pkAttributes,
			boolean reloadModelWhenSelectingOnGrid) {
		linkGrid(grid, pkAttributes, reloadModelWhenSelectingOnGrid,
				reloadModelWhenSelectingOnGrid, reloadModelWhenSelectingOnGrid,
				null);
	}

	/**
	 * Link the specified grid control to the current Form, so that: - grid
	 * navigator bar event (i.e. pressing one of its buttons) on grid will force
	 * the Form data loading (e.g. when using grid navigator bar) - insert new
	 * data on the Form will refresh grid by adding a new row - update data on
	 * Form will refresh grid for the related changed row - delete existing data
	 * on the Form will refresh grid content by removing the related row To
	 * correctly identify the row on grid to change a matching beetween
	 * attributes (specified in "pkAttributes" argument) of grid v.o. and Form
	 * v.o. is performed: it will be identified the first row on grid that
	 * matches this filter criteria, based on attribute values. NOTE: if more
	 * than one Form has been opened, then all Forms will be reloaded to the
	 * same row when a grid navigator button is being pressed.
	 * 
	 * In addition, Form data loading is automatically performed when using the
	 * specified navigator bar; also grid selection row is updated. NOTE:
	 * "reloadModelWhenSelectingOnGrid" is set to <code>true</code> then Form
	 * data reloading is performed also when clicking with the left mouse button
	 * onto the grid and when pressing up/down keys onto the grid.
	 * 
	 * @param grid
	 *            grid control linked to the current Form, to update grid
	 *            content
	 ...
	 *            row selection on grid by grid navigator bar
	 * @param navBar
	 *            navigation bar linked to the current Form, used to load data
	 *            on Form according to the selected row on grid, whose selection
	 *            has been changed when using this navigator bar
	 */
	public final void linkGrid(GridControl grid, HashSet pkAttributes,
			boolean reloadModelWhenSelectingOnGrid, NavigatorBar navBar) {
		linkGrid(grid, pkAttributes, reloadModelWhenSelectingOnGrid,
				reloadModelWhenSelectingOnGrid, reloadModelWhenSelectingOnGrid,
				navBar);
	}
	 ...
	 * 
	 * In addition, Form data loading is automatically performed when using the
	 * specified navigator bar; also grid selection row is updated.
	 * 
	 * @param grid
	 *            grid control linked to the current Form, to update grid
	 *            content
	 ...
	 *            <code>true</code> to force data Form reloading when changing
	 *            row selection on grid by grid navigator bar
	 * @param reloadModelWhenClickingWithMouse
	 *            <code>true</code> to enable Form data reloading when clicking
	 *            with the left mouse button onto the grid
	 * @param reloadModelWhenPressingKey
	 *            <code>true</code> to enable Form data reloading when pressing
	 *            up/down keys onto the grid
	 * @param navBar
	 *            navigation bar linked to the current Form, used to load data
	 *            on Form according to the selected row on grid, whose selection
	 *            has been changed when using this navigator bar
	...
	public final void linkGrid(GridControl grid, HashSet pkAttributes,
		...
		if (grid != null && pkAttributes != null) {
			if (grid.getNavBar() != null && reloadModelWhenSelectingOnGrid) {
				// if there exist a navigator bar linked to the grid, then
				// listen events from it...
				...
				this.addAncestorListener(new AncestorListener() {
					...
					public void ancestorRemoved(AncestorEvent event) {
						if (Form.this.grid != null
								&& Form.this.grid.getNavBar() != null) {
							Form.this.grid.getNavBar()
									.removeAfterActionListener(Form.this);
						}
					}
				});
	...
	/**
	 * @return row index in grid related to the linked grid's v.o. having the
	 *         same pk of the current Form v.o.; -1 if that row does not exist
	 *         in grid or when the grid has not been linked to this Form (see
	 *         linkGrid method))
	 */
	public final int getRowIndexInGrid() {
		...
		if (grid != null && pkAttributes != null) {
			...
			for (int i = 0; i < grid.getVOListTableModel().getRowCount(); i++) {
				...
				while (it.hasNext()) {
					pkAttrName = it.next().toString();
					colIndex = grid.getVOListTableModel()
							.findColumn(pkAttrName);
					if (colIndex == -1) {
						rowFound = false;
						break;
					}
	...
	/**
	 * Callback method invoked by the navigator bar of the grid when the user
	 * has pressed a button on it.
	 */
	public final void actionPerformed(ActionEvent e) {
		// a row on the linked grid has been just selected: reload data model
		// for this Form...
		if (e != null
				&& e.getSource() != null
				&& navBar != null
				&& e.getSource().equals(navBar)
				|| (grid != null
						&& grid.getNavBar() != null
						&& e.getSource().equals(grid.getNavBar())
						&& !(!reloadModelWhenClickingWithMouse && e
								.getActionCommand().equals(
										NavigatorBar.LEFT_MOUSE_BUTTON))
						&& !(!reloadModelWhenPressingKey && e
								.getActionCommand().equals(NavigatorBar.UP_KEY)) && !(!reloadModelWhenPressingKey && e
						.getActionCommand().equals(NavigatorBar.DOWN_KEY))))
			reload();
	}

	/**
	 * @return define if an inner v.o. must be automatically instantiated when a
	 *         setter method is invoked
	 */
	...
	/**
	 * Define if an inner v.o. must be automatically instantiated when a setter
	 * method is invoked.
	 * 
	 * @param createInnerVO
	 *            define if an inner v.o. must be automatically instantiated
	 *            when a setter method is invoked
	 */
	...
	/**
	 * @return collection of buttons binded to grid (InsertButton, EditButton,
	 *         etc)
	 */
	 ...
	 * </p>
	 * <p>
	 * Description: Inner class used to listen for Form's input controls focus
	 * events.
	 * </p>
	 * 
	 * @version 1.0
 ...
 * </p>
 * <p>
 * Description: Inner class that stores a couple (attribute name, attribute
 * value).
 * </p>
 * 
 * @author Mauro Carniel
