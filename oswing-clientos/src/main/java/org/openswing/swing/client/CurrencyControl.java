 ...
 * </p>
 * <p>
 * Description: Currency (numeric) input control.
 * </p>
 * <p>
...
public class CurrencyControl extends BaseInputControl implements InputControl {
	...
	/**
	 * flag used to define whether zero digits (after decimal point) must be
	 * hided/showed; default value:
	 * <code>ClientSettings.ZERO_SHOWS_AS_ABSENT</code> i.e. show zero digits
	 */
	private boolean hideZeroDigits = ClientSettings.HIDE_ZERO_DIGITS;

	/**
	 * currency symbol; default value:
	 * ClientSettings.getInstance().getResources().getCurrencySymbol()
	 */
	private String currencySymbol = ClientSettings.getInstance().getResources()
			.getCurrencySymbol();

	/**
	 * decimal symbol; default value:
	 * ClientSettings.getInstance().getResources().getDecimalSymbol()
	 */
	private char decimalSymbol = ClientSettings.getInstance().getResources()
			.getDecimalSymbol();

	/**
	 * grouping symbol; default value:
	 * ClientSettings.getInstance().getResources().getGroupingSymbol()
	 */
	private char groupingSymbol = ClientSettings.getInstance().getResources()
			.getGroupingSymbol();

	/**
	 * flag used to define the default position of currency symbol in currency
	 * control/column: on the left or on the right of the numeric value; default
	 * value: <code>ClientSettings.CURRENCY_SYMBOL_ON_LEFT</code> i.e. on the
	 * left of the numeric value
	 */
	 ...
	 * 
	 * @param columns
	 *            number of visibile characters
	 */
	public CurrencyControl(int columns) {
		currencyBox.setColumns(columns);
		currencyBox.setDisabledTextColor(UIManager
				.getColor("TextField.foreground"));

		// setLayout(new FlowLayout(FlowLayout.LEFT, 0, 0));
		// this.add(currencyBox);
		this.setLayout(new GridBagLayout());
		this.add(currencyBox, new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0,
				GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL,
				new Insets(0, 0, 0, 0), 0, 0));
		// this.setLayout(new java.awt.BorderLayout(0,0));
		// this.add(currencyBox, java.awt.BorderLayout.CENTER);

		StringBuffer s = new StringBuffer(currencyBox.getColumns());
		for (int i = 0; i < currencyBox.getColumns(); i++)
			s.append("0");
		setMinimumSize(new Dimension(currencyBox.getFontMetrics(
				currencyBox.getFont()).stringWidth(s.toString()),
	...
	protected void setFormat() {
		...
		if (!isGrouping() && getDecimals() == 0) {
			...
			if (currencySymbolOnLeft)
				...
				format = new DecimalFormat("#,##0." + dec + " "
						+ currencySymbol, dfs);
		} else if (!isGrouping() && getDecimals() > 0) {
			String dec = "";
			for (int i = 0; i < getDecimals(); i++)
				dec += zero;
			if (currencySymbolOnLeft)
				format = new DecimalFormat(currencySymbol + " " + "0." + dec,
						dfs);
			else
				format = new DecimalFormat("0." + dec + " " + currencySymbol,
						dfs);
		}
	...
	/**
	 * @return flag used to define the default position of currency symbol in
	 *         currency control/column: on the left or on the right of the
	 *         numeric value
	 */
	...
	/**
	 * Set the flag used to define the default position of currency symbol in
	 * currency control/column: on the left or on the right of the numeric
	 * value. Default value: <code>ClientSettings.CURRENCY_SYMBOL_ON_LEFT</code>
	 * i.e. on the left of the numeric value.
	 * 
	 * @param currencySymbolOnLeft
	 *            flag used to define the default position of currency symbol in
	 *            currency control/column: on the left or on the right of the
	 *            numeric value
	 */
	...
	public final BigDecimal getBigDecimal() {
		if (currencyBox.getText().length() == 0
				|| currencyBox.getText().equals(nullValue))
			return null;
		try {
			BigDecimal num = new BigDecimal(format.parse(currencyBox.getText())
					.doubleValue());
			// if
			// (String.valueOf(num.doubleValue()-(double)num.intValue()).length()>10)
			num = num.setScale(decimals, BigDecimal.ROUND_HALF_UP);
			return num;
		} catch (ParseException ex) {
			Logger.error(this.getClass().getName(), "getBigDecimal",
					"Error while creating BigDecimal object", ex);
			return null;
		}
	}
	...
	public final Double getDouble() {
		...
		try {
			...
			Logger.error(this.getClass().getName(), "getDouble",
					"Error while creating BigDecimal object", ex);
			return null;
		}
	}
	...
	public final String getText() {
		...
		try {
			if (currencyBox.getText().length() != 0
					&& !currencyBox.getText().trim().equals(nullValue.trim())
					&& !"-".equals(currencyBox.getText()))
				value = new BigDecimal(format.parse(currencyBox.getText())
						.doubleValue());
			if ("-".equals(currencyBox.getText()))
				value = new BigDecimal(0).negate();
		} catch (ParseException ex) {
			Logger.error(this.getClass().getName(), "getText",
					"Error while creating BigDecimal object", ex);
			return null;
		}
	...
	private void addKeyListener() {
		currencyBox.addKeyListener(new KeyAdapter() {
			...
			public final void keyTyped(KeyEvent e) {
				...
				if (e.getKeyChar() == ClientSettings.getInstance()
						.getResources().getDecimalSymbol()
						&& decimals > 0
						&& currencyBox.getText() != null
						&& currencyBox.getText().indexOf(
				...
				if (e.getKeyChar() == ClientSettings.getInstance()
						...
						&& decimals > 0)
					return;
				else if (e.getKeyChar() == ClientSettings.getInstance()
						.getResources().getGroupingSymbol()
						&& grouping)
					return;
				else if (e.getKeyChar() == '\b') {
					if (currencySymbolOnLeft
							&& currencyBox.getCaretPosition() <= nullValue
									.length())
						e.consume();
					else if (!currencySymbolOnLeft
							&& currencyBox.getCaretPosition() > currencyBox
									.getText().length() - nullValue.length())
						e.consume();

					return;
				} else if (e.getKeyChar() == '-'
						&& minValue < 0
						&& currencyBox.getCaretPosition() == 0
						&& currencyBox.getText() != null
						&& (currencyBox.getText().length() == 0 || currencyBox
								.getText().charAt(0) != '-'))
					return;
			...
			public void keyPressed(KeyEvent e) {
				...
				if (e.getKeyCode() == e.VK_ENTER || e.getKeyCode() == e.VK_TAB
						...
						|| e.getKeyCode() == e.VK_RIGHT || e.isAltDown())
					return;
				e.consume();
			}
			...
			public void focusLost(FocusEvent e) {
				...
				if (currencyBox.isEnabled() && currencyBox.isEditable()) {
					...
					if (currencyBox.getText().length() > maxCharacters)
						CurrencyControl.this.setText(currencyBox.getText()
	...
	/**
	 * @return flag used to define whether zero digits (after decimal point)
	 *         must be hided/showed
	 */
	...
	/**
	 * Define whether zero digits (after decimal point) must be hided/showed;
	 * default value: <code>false</code> i.e. show zero digits.
	 * 
	 * @param hideZeroDigits
	 *            flag used to define whether zero digits (after decimal point)
	 *            must be hided/showed
	 */
	 ...
	 * 
	 * @param enabled
	 *            flag used to set abilitation of control
	 */
	public void setEnabled(boolean enabled) {
		currencyBox.setEditable(enabled);
		currencyBox.setFocusable(enabled
				|| ClientSettings.DISABLED_INPUT_CONTROLS_FOCUSABLE);
		if (!enabled) {
			currencyBox.setBackground((Color) UIManager
					.get("TextField.inactiveBackground"));
		} else {
			if (isRequired() && ClientSettings.PAINT_BG_MANDATORY_FIELD) {
				currencyBox.setForeground(UIManager
						.getColor("TextField.foreground"));
				currencyBox
				...
				currencyBox.setForeground(UIManager
						.getColor("TextField.foreground"));
				currencyBox.setBackground(UIManager
	...
	/**
	 * Removes the specified key listener so that it no longer receives action
	 ...
	 * </p>
	 * <p>
	 * Description: Inner class used to redirect key event to the inner
	 * JTextField.
	 * </p>
	 * <p>
	 * Copyright: Copyright (C) 2006 Mauro Carniel
	...
	class CurrencyBox extends JTextField {
				...
				public void insertUpdate(DocumentEvent e) {
					if (!currencySymbolOnLeft
							&& getCaretPosition() >= getText().length()
									- nullValue.length()) {
						// currency symbol must be showed on the right of
						// numeric value and
						// user attempts to write on the right of currency
						// symbol
						try {
					...
					if (!currencySymbolOnLeft
							&& getText().startsWith(nullValue))
						// currency symbol must be showed on the right of
						// numeric value and
						// current numeric value is null and user attempts to
						// write the first digit:
						// without rewriting just editing data, it would be
						// write on the right of the text field and
						// this is not correct...
						try {
							final BigDecimal n = new BigDecimal(getText()
									.substring(nullValue.length()));
							SwingUtilities.invokeLater(new Runnable() {
								public void run() {
									setText(n + nullValue);
									setCaretPosition(getText().length()
		...
		public String getText() {
			...
			if (t != null && !currencySymbolOnLeft && t.indexOf(" ") > 0)
		...
		public void setCaretPosition(int pos) {
			if (currencySymbolOnLeft && pos < nullValue.length())
				super.setCaretPosition(nullValue.length());
					...
					&& pos >= getText().length() - nullValue.length())
