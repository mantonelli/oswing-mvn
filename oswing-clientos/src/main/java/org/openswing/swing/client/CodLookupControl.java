 ...
 * </p>
 * <p>
 * Description: Lookup input Control used to digit a code. A code can be an
 * alphanumeric or a numeric value. When focus is lost from this control, a code
 * validation is executed (only if the specified code is changed). It contains
 * also a lookup button, i.e. a button which allows to view a lookup grid of
 * codes: the user can select a code from the lookup grid and this code will be
 * set in the code input field. It optionally contains also a "+" button, used
 * to call a controller class, related to the code registry, i.e. the registry
 * that allows to insert/update/delete codes referred by the lookup.
 * </p>
 * <p>
...
public class CodLookupControl extends BaseInputControl implements
	...
	private boolean showButtonSeparator = false;
	...
	private JButton lookupButton = new JButton() {
		public void paint(Graphics g) {
			...
			g.drawString("...", (this.getWidth() - width + 1) / 2,
					this.getHeight() / 2 + 4);
		}
	};
	...
	/**
	 * lookup controller: this class will execute the code validation and the
	 * lookup grid opening
	 */
	private LookupController validationController = null;

	/**
	 * class name of the controller that must be invoked by pressing the "+"
	 * button
	 */
	private String controllerClassName = null;

	/**
	 * method name defined in ClientFacade class, related to the controller that
	 * must be invoked by pressing the "+" button
	 */
	private String controllerMethodName = null;

	/**
	 * "+" button, used to call the controller class, related to the code
	 * registry
	 */
	...
	/**
	 * wait time (expressed in ms) before showing code auto completition feature
	 * for lookup controls; default value:
	...
	public CodLookupControl() {
		...
		setColumns(10);
		setShowButtonSeparator(false);

		lookupButton.setText(null);
		lookupButton.setPreferredSize(new Dimension(21, codBox
		...
		lookupButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (validationController != null)
					validationController.openLookupFrame(lookupButton,
							CodLookupControl.this);
			}
		...
		plusButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (controllerClassName != null) {
					...
					try {
						MDIFrame.getClientFacade()
								.getClass()
								.getMethod(controllerMethodName, new Class[0])
								.invoke(MDIFrame.getClientFacade(),
										new Object[0]);
					} catch (Throwable ex) {
						ex.printStackTrace();
					}
		...
		this.add(codBox, new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0,
				GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL,
				new Insets(0, 0, 0, 0), 0, 0));
		this.add(buttonSeparator, new GridBagConstraints(1, 0, 1, 1, 0.0, 1.0,
				GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0,
						0, 0, 0), 0, 0));
		this.add(lookupButton, new GridBagConstraints(2, 0, 1, 1, 0.0, 0.0,
				GridBagConstraints.WEST, GridBagConstraints.VERTICAL,
				new Insets(0, 0, 0, 0), 0, 0));

		lookupButton.setPreferredSize(new Dimension(lookupButton
				.getPreferredSize().width, codBox.getPreferredSize().height));
		lookupButton.setMaximumSize(new Dimension(lookupButton
				.getPreferredSize().width, codBox.getPreferredSize().height));
		lookupButton.setMinimumSize(new Dimension(lookupButton
				.getPreferredSize().width, codBox.getPreferredSize().height));

		plusButton.setPreferredSize(new Dimension(lookupButton
				.getPreferredSize().width, codBox.getPreferredSize().height));
		plusButton.setMaximumSize(new Dimension(
				lookupButton.getPreferredSize().width, codBox
						.getPreferredSize().height));
		plusButton.setMinimumSize(new Dimension(
				lookupButton.getPreferredSize().width, codBox
						.getPreferredSize().height));
	 ...
	 * 
	 * @param code
	 *            used to retrieve the corresponding item and to select that
	 *            item in the combo
	 */
	...
	public final void setColumns(int columns) {
		...
		for (int i = 0; i < columns; i++)
			s.append("W");
		int w = getFontMetrics(getFont()).stringWidth(s.toString())
	...
	/**
	 * Method that listen validation code event fired by the code input field:
	...
	public final void validateCode(String code)
		...
		if (autoCompletitionWaitTime >= 0 && autoCompletitionListener != null
		...
		if (validationController != null)
			validationController.validateCode(codBox, code, this);
	}
	...
	public final void setEnabled(boolean enabled) {
		...
		this.codBox.setFocusable(enabled
				|| ClientSettings.DISABLED_INPUT_CONTROLS_FOCUSABLE);
	...
	/**
	 * Set lookup controller. This method calls also addLookupListener method.
	...
	public final void addNotify() {
		...
		if (firstTime) {
			...
			// set Form object in the lookup container...
			if (!Beans.isDesignTime() && validationController != null
					&& validationController.getForm() == null) {
				Form form = ClientUtils.getLinkedForm(this);
				if (form != null)
			...
			if (!Beans.isDesignTime() && autoCompletitionWaitTime >= 0
				...
				autoCompletitionListener = new AutoCompletitionListener(this,
						new LookupAutoCompletitionDataLocator(
								validationController, getAttributeName()),
			...
			codBox.addKeyListener(new KeyAdapter() {
				public void keyReleased(KeyEvent e) {
					if (e.getKeyCode() == ClientSettings.LOOKUP_OPEN_KEY
							.getKeyCode()
							&& e.getModifiers() + e.getModifiersEx() == ClientSettings.LOOKUP_OPEN_KEY
									.getModifiers()) {
						if (validationController != null)
							validationController.openLookupFrame(codBox,
									CodLookupControl.this);
					}
					if (e.getKeyCode() == ClientSettings.LOOKUP_CONTROLLER_KEY
							.getKeyCode()
							&& e.getModifiers() + e.getModifiersEx() == ClientSettings.LOOKUP_CONTROLLER_KEY
									.getModifiers()) {
						if (controllerClassName != null) {
							try {
								Class.forName(controllerClassName)
										.newInstance();
							} catch (Throwable ex) {
								ex.printStackTrace();
							}
							...
							try {
								MDIFrame.getClientFacade()
										.getClass()
										.getMethod(controllerMethodName,
												new Class[0])
										.invoke(MDIFrame.getClientFacade(),
												new Object[0]);
							} catch (Throwable ex) {
	...
	private final void addLookupListener(LookupController controller) {
		...
		controller.addLookupListener(new LookupListener() {
			...
			/**
			 * This method is called before code validation and when pressing
			 * lookup button.
			 */
			 ...
			 * 
			 * @param parentVO
			 *            v.o. of the parent lookup container
			 * @param parentChangedAttributes
			 *            changed attributes of the v.o. of the parent lookup
			 *            container
			 */
				...
				try {
					...
					while (list.hasNext()) {
						...
						if (parentVO != null) {
							...
							if (obj == null)
								newValue = null;
							else if (ClientUtils.getPropertyDescriptor(
									obj.getClass(), aux) == null)
								newValue = null;
							else
								newValue = ClientUtils
						...
						ValueChangeEvent e = new ValueChangeEvent(this,
								attrName, null, newValue);
	 ...
	 * 
	 * @param attributeName
	 *            attribute name in the parent v.o. that must be updated
	 * @param value
	 *            updated value
	 */
	public void setValue(String attributeName, Object value) {
		if (validationController.getForm() != null)
			validationController.getForm().getVOModel()
	...
	public Object getLookupCodeParentValue() {
		if (validationController.getForm() != null
				&& validationController.getForm().getVOModel() != null
				&& getAttributeName() != null)
			return validationController.getForm().getVOModel()
					.getValue(getAttributeName());
		return "";
	}

	/**
	 * @return class name of the controller that must be invoked by pressing the
	 *         "+" button
	 */
	...
	/**
	 * @return method name defined in ClientFacade class, related to the
	 *         controller that must be invoked by pressing the "+" button
	 */
	...
	/**
	 * Set the class name of the controller that must be invoked by pressing the
	 * "+" button.
	 * 
	 * @param controllerClassName
	 *            class name of the controller that must be invoked by pressing
	 *            the "+" button
	 */
	public final void setControllerClassName(String controllerClassName) {
		this.controllerClassName = controllerClassName;
		if (controllerMethodName != null)
			this.add(plusButton, new GridBagConstraints(3, 0, 1, 1, 0.0, 0.0,
					GridBagConstraints.WEST, GridBagConstraints.VERTICAL,
					new Insets(0, 0, 0, 0), 0, 0));
	}

	/**
	 * Set the method name defined in ClientFacade class, related to the
	 * controller that must be invoked by pressing the "+" button.
	 * 
	 * @param controllerMethodName
	 *            method name defined in ClientFacade class, related to the
	 *            controller that must be invoked by pressing the "+" button
	 */
	...
	/**
	 * @return wait time (expressed in ms) before showing code auto completition
	 *         feature for lookup controls; <code>-1</code>, to do not enable
	 *         auto completition
	 */
	...
	/**
	 * Wait time before showing code auto completition feature for this lookup
	 * control.
	 * 
	 * @param autoCompletitionWaitTime
	 *            wait time (expressed in ms) before showing code auto
	 *            completition feature for this lookup control; default value:
	 *            <code>-1</code> to do not enable auto completition
	 */
	...
	public void setShowButtonSeparator(boolean show) {
		this.showButtonSeparator = show;
		buttonSeparator.setVisible(show);
	}

	public boolean getShowButtonSeparator() {
		return showButtonSeparator;
	}
}
